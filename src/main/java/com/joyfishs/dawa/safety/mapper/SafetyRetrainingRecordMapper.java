...
public interface SafetyRetrainingRecordMapper extends BaseMapper<SafetyRetrainingRecord> {
     ...
     * 查询人员待完成的再培训记录
     */
    @Select("SELECT * FROM t_safety_retraining_record WHERE person_id = #{personId} AND status = 'pending' ORDER BY created_at DESC")
    List<SafetyRetrainingRecord> selectPendingByPersonId(@Param("personId") Long personId);

    /**
     * 查询人员所有待完成的再培训记录
     */
    @Select("SELECT * FROM t_safety_retraining_record WHERE person_id = #{personId} AND status IN ('pending', 'ongoing') ORDER BY created_at DESC")
    List<SafetyRetrainingRecord> selectPendingOrOngoingByPersonId(@Param("personId") Long personId);

    /**
     * 更新状态
     */
    @Update("UPDATE t_safety_retraining_record SET status = #{status}, updated_at = NOW() WHERE id = #{id}")
    int updateStatus(@Param("id") Long id, @Param("status") String status);

    /**
     * 标记为已完成
     */
    @Update("UPDATE t_safety_retraining_record SET status = 'completed', end_date = #{endDate}, score_restored = #{scoreRestored}, updated_at = NOW() WHERE id = #{id}")
